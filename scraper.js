  ...
  for (const { checkIn, checkOut } of dates) {
    for (const hotel of hotels) {
      ...
      const url = [
        ...
        `?action=price&tid=211&idt=`,
        `&flt2=${hotel.flt2}`,
        `&id_price=${hotel.id_price}`,
        `&data=${checkIn}&d2=${checkOut}`,
        `&f7=7&f3=&f8=&ho=0`,
        f1Part,
        `&F4=${hotel.hotelId}`,
        `&ins=0-40000-EUR`,
      ...
      urls.push({ url, checkIn, hotelId: hotel.hotelId });
    }
  }
  return urls;
}
...
async function sendTelegram(text) {
  if (!TELEGRAM_BOT_TOKEN || !TELEGRAM_CHAT_ID) {
    console.log('[TEL]', text.slice(0, 120));
    return;
  }
...
async function sendTelegramSplit(aheadAlerts, equalAlerts) {
  ...
  for (const g of Object.values(groups)) {
    ...
    if ((current + block).length > 3500) {
      ...
      current = '🏨 <b>Fiyat Raporu (devam)</b>\n\n' + block;
    } else {
      current += block;
    }
  }
...
async function main() {
  ...
  try {
    ...
    for (const [checkIn, offers] of Object.entries(offersByDate)) {
      console.log(`  [${checkIn}] ${offers.length} otel blogu`);
...
main().catch(async err => {
  ...
  await sendTelegram(`❌ <b>Monitor Hatasi</b>\n\n${err.message}`).catch(() => {});
  process.exit(1);
});
